...
class ModuleRedeclarator(object):
    ...
    def redo_class(self, out, p_class, p_name, indent, p_modname=None, seen=None, inspect_dir=False, used_imports=None):
        ...
        if properties:
            ...
            #
        class_doc = getattr(p_class, "__doc__", None)
        for item_name in sorted_no_case(others.keys()):
            item = others[item_name]
            self.fmt_value(out, item, indent + 1, prefix=item_name + " = ")
            # search for enum values documentation in class docstring, except for placeholder first/count values
            if isinstance(p_class, enum.EnumMeta) and item_name not in ['First', 'Count']:
                offs = 0
                while True:
                    result = DATA_PATTERN.search(class_doc, offs)
                    # stop if there are no more datas - we should have one for each, but don't fail
                    if result is None:
                        print(f"Couldn't find enum docstring for {item_name}")
                        break
                    ...
                    if data_decl == item_name:
                        ...
                        enum_doc_start = result.end(1)
                        next_result = DATA_PATTERN.search(class_doc, enum_doc_start)
                        # if there isn't, the docstring is the remainder of the class doc,
                        # otherwise the docstring stops at the next data
                        if next_result is None:
                            enum_doc = class_doc[enum_doc_start:]
                        else:
                            enum_doc = class_doc[enum_doc_start:next_result.start(0)]

                        out_docstring(out, enum_doc.strip(), indent + 1)
                        out(0, "") # empty line after docstring
                        break
                    offs = result.start(1)
        if p_name == "object":
    ...
    def redo(self, p_name, inspect_dir):
        ...
        if classes:
            ...
            self.split_modules = True # always output split modules
